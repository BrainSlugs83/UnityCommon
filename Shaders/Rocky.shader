Shader "BrainSlugs83/Rocky"
{
	Properties
	{
		...
		_Discoloration("Discoloration", Range(-2, 2)) = 0.1
	...
	SubShader
	{
		...
		float3 DistortPosition(float3 positionOS, out float discoloration)
		{
			float3 worldPosition = TransformObjectToWorld(positionOS);
			// Potential Bug #37: This is a bug because _CellSize permits zero and dividing worldPosition by zero feeds infinity or NaN values into the noise and vertex-displacement calculations.
			// Suggested Fix: Divide by max(abs(_CellSize), a small positive epsilon), or change the property range so zero cannot be selected.
			// Related: None.
			float3 value = worldPosition / _CellSize;
			float3 noise = ValueNoise3Dto3D(value);
			discoloration = (ValueNoise3Dto1D(value) * _Discoloration) - (_Discoloration * .5);
			return positionOS + noise * _DistortionStrength;
		}
		...
		ForwardVaryings ForwardVertex(Attributes input)
		{
			...
			float discoloration;
			input.positionOS.xyz = DistortPosition(input.positionOS.xyz, discoloration);
			...
			output.discoloration = discoloration;
		...
		half4 ForwardFragment(ForwardVaryings input) : SV_Target
		{
			...
			half4 albedo =
				SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, TRANSFORM_TEX(input.uv, _MainTex)) * _Color;
			...
			half4 ao = SAMPLE_TEXTURE2D(_AoTex, sampler_AoTex, TRANSFORM_TEX(input.uv, _AoTex));
		...
		half4 RockyMetaFragment(MetaVaryings input) : SV_Target
		{
			half3 albedo =
				SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, TRANSFORM_TEX(input.uv, _MainTex)).rgb *
				_Color.rgb;
		...
		Pass
		{
			...
			#pragma fragment RockyMetaFragment
